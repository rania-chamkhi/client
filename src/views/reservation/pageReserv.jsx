...
const Reservation = () => {
 ...
 if (!data.dateReserv) {
    Swal.fire('Erreur', 'Veuillez remplir le champ "Pris le".', 'error');
    return;
  }

  if (!data.heureResrv) {
    Swal.fire('Erreur', 'Veuillez sélectionner l\'heure de réservation.', 'error');
    return;
  }

  if (!data.dateFinloc) {
    Swal.fire('Erreur', 'Veuillez remplir le champ "Remis le".', 'error');
    return;
  }

  if (!data.heureRet) {
    Swal.fire('Erreur', 'Veuillez remplir le champ "Heure".', 'error');
    return;
  }

  if (!data.numPermis) {
    Swal.fire('Erreur', 'Veuillez remplir le champ "Numéro de permis".', 'error');
    return;
  }
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
       await reservationService.createReser(data).then(async (res) => {
          console.log("createReser ",res)

         await reservationService.createNot().then(async resnot=> { 

             console.log("res of creqte noif qfter res ",resnot)
          }).catch((err) => {
  ...
  useEffect(() => {
    ...
    if (data.dateReserv && dateFinloc) {
      ...
      if (assuranceId) {
        ...
        console.log(selectedAssurancePrice)
      }
